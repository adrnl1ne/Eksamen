...
public class LejeaftaleRepo {
    ...
    //Jakob
  /*  public boolean isLejeperiodeOverstået(LejeAftale lejeAftale) {
        int lejeaftale_ID = lejeAftale.getLejeAftale_ID();
        boolean isOverstået;
        LocalDate slutdato = null;

        try {
            String startdato_QUERY = "SELECT StartDato FROM lejeaftale WHERE Lejeaftale_ID=?";
            PreparedStatement preparedStatement = DCM.prepareStatement(startdato_QUERY);
            preparedStatement.setInt(1, lejeaftale_ID);
            ResultSet resultSet = preparedStatement.executeQuery();

            if (resultSet.next()) {
                slutdato = resultSet.getDate("StartDato").toLocalDate();
            }
        } catch (SQLException e) {
            System.err.println("Kan ikke finde lejeaftalen:" + lejeaftale_ID);
            e.printStackTrace();
            throw new RuntimeException(e);
        }

        if (slutdato.isEqual(lejeAftale.getSlutdatoDefault())) {
            isOverstået = true;
        } else {
            isOverstået = false;
        }
